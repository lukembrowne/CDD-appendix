...
# Create column for mortality: 1 == dead, 0 == alive
data_BCI$mortality <- ifelse(data_BCI$status == 1, yes = 0, no = 1)
...
model_fit = function(data, speciesinfo, reduced = F) {
  ...
  if (reduced) {
    form =  as.formula(paste0("mortality ~ s(s_height, k = k1) + s(total_dens, k = k2)"
                              , term_c))
  } else {
    form =  as.formula(paste0("mortality ~ s(s_height, k = k1) + s(total_dens, k = k2)  + s(con_dens, k = k3)"
                              , term_c))
  }
...
mod <- gam(formula = mortality ~ s(s_height, k = k1) + s(total_dens, k = k2) + s(con_dens, k = k3),
               family = binomial(link = cloglog),
               offset = log(interval),
               data = data_BCI_1,
               method = "REML")
