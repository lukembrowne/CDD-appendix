...
This tutorial follows from Section 2. "Modeling considerations when estimating CDD" of the main text. ## 3.1 Overview
...
Note: The code is adapted from [@hulsmann2024latitudinal] and the latitudinalCNDD repository by Lisa Hülsmann.
...
The cloglog link allows accounting for differences in observation time Δ𝑡 through an offset term, applicable to datasets where (0=alive and 1=dead)[@currie2016fitting].In other words, to use a cloglog link to account for differences in census interval length, you must be modeling mortality, not survival. (more about cloglog link here - section 3.2)
