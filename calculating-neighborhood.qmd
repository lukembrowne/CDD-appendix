...
We then demonstrate how to weight the calculation of neighborhood density by individual size (*i.e.,* basal area) and distance using an exponential decay function (one of many possible decay functions), allowing the competitive effects of density effects to saturate [@uriarte2010trait; @canham2006neighborhood].
...
decay_values <- seq(from = 5, to = 10, by = 5)
...
# Subset to first census - this will be different for different datasets
...
Next we add in additional columns for neighbor characteristic, (*e.g.,* species, size) and whether they are located near the edge of the plot (blue area in @fig-edge).
...
neighbors_summary_total = bind_rows(neighbors_summary_long_format,
                        ...
                        #  filter(comparison_type != "het") %>% # Can filter out heterospecific neighborhood values by uncommenting this line of the objects become too large
...
#| fig-cap: "Heatmap of optimal values for decay constants separately for each species"
  ...
  sums <- sums %>%
    group_by(sp) %>%
    mutate(logLik_scaled = scale(logLik)) %>% # Scale loglikihood by species to help with visualization
    ungroup()
...
# Plot heatmap of log likelihood values
  ggplot(sums, aes(x = decay_total, y = decay_con, fill = logLik_scaled)) +
    geom_tile(width = 0.9, height = 0.9, col = "black") +
    geom_label(data = optimum_by_sp, label = "X") +
    labs(x = "Decay total density", y = "Decay conspecific density", fill = "logLik") +
