...
knitr::kable(head(sums %>% select(run_name, df, logLik, AIC, nobs)), digits = 2) %>%
  kable_styling(latex_options = c("striped", "scale_down", "smaller"))
```
...
```{r}
knitr::kable(optimum_by_sp %>% select(run_name, logLik, AIC), digits = 2) %>%
  kable_styling(latex_options = c("striped", "scale_down"))
```
